(() => {
  ...
  style.textContent = `
    ...
    nav.nav-hidden {
      ...
      top: var(--nav-top, 24px) !important;
      left: 50% !important;
      width: min(var(--nav-width, 1180px), calc(100% - 28px)) !important;
      height: var(--nav-height, 72px) !important;
      margin: 0 !important;
      padding: 7px 8px 7px 14px !important;
      transform: translateX(-50%) scale(var(--nav-scale, 1)) !important;
      ...
      border: 1px solid rgba(238,205,255,var(--nav-border-alpha,.09)) !important;
      border-radius: var(--nav-radius, 28px) !important;
      background: rgba(7,4,12,var(--nav-bg-alpha,.28)) !important;
      background-image: none !important;
      backdrop-filter: blur(var(--nav-blur, 14px)) saturate(150%) !important;
      -webkit-backdrop-filter: blur(var(--nav-blur, 14px)) saturate(150%) !important;
      ...
      will-change: width,top,height,transform,background,border-radius,box-shadow,backdrop-filter !important;
    }
    ...
    .hero.nexus-hero { --hero-shift:0px; --hero-fade:1; }
    .hero.nexus-hero .hero-copy { transform:translate3d(0,var(--hero-shift),0) scale(var(--hero-copy-scale,1)) !important; opacity:var(--hero-fade,1) !important; filter:blur(var(--hero-blur,0px)) !important; transform-origin:left center; will-change:transform,opacity,filter; }
    ...
    @media(max-width:760px){
      nav,nav.nav-docked,nav.nav-fading,nav.nav-hidden{top:8px!important;height:58px!important;border-radius:19px!important;padding:5px 8px!important;width:calc(100% - 16px)!important;transform:translateX(-50%)!important}
      .hero.nexus-hero .hero-copy,.hero.nexus-hero .hero-console{filter:none!important;opacity:1!important}
    }
  `;
  ...
  function applyScrollMorph() {
    ...
    const raw = clamp01(window.scrollY / 220);
    const p = ease(raw);
    ...
    if (hero) {
      const hp = clamp01(window.scrollY / 520);
      const he = ease(hp);
  ...
  function goTo(target, hash) {
    ...
    if (reduceMotion || Math.abs(distance) < 6) {
      window.scrollTo(0, end); history.replaceState(null, '', hash); lockedHash = null; applyScrollMorph(); scrollActive(); return;
    }

    const duration = Math.min(420, Math.max(190, Math.abs(distance) * .12));
    const began = performance.now();
    const scrollEase = t => 1 - Math.pow(1 - t, 3);
    const tick = now => {
      const p = Math.min(1, (now - began) / duration);
      window.scrollTo(0, start + distance * scrollEase(p));
      applyScrollMorph();
      setActive(hash);
      if (p < 1) raf = requestAnimationFrame(tick);
      else {
        history.replaceState(null, '', hash);
        lockTimer = setTimeout(() => { lockedHash = null; scrollActive(); }, 180);
      }
    };
  ...
  guard.observe(nav, { attributes:true, attributeFilter:['class'] });
  ...
  window.addEventListener('resize', applyScrollMorph, { passive:true });
